...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[41] === p.contractAddress[41])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'TLOS'))(
    ...
    async (pool) => {
      ...
      expect(rewardTokenAddress.toLowerCase()).toBe(pool.earningToken.address[41].toLowerCase())
    },
  )
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'TLOS'))(
    ...
    async (pool) => {
      ...
      expect(stakingTokenAddress.toLowerCase()).toBe(pool.stakingToken.address[41].toLowerCase())
    },
  )
})
