...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.TESTNET]).toBeTruthy()
      expect(txs[ChainId.TESTNET]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.TESTNET]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.TESTNET)])
      expect(Object.keys(store.getState()[ChainId.TESTNET] ?? {})).toEqual(['0x0'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.TESTNET }))
      expect(Object.keys(store.getState())).toHaveLength(1)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.TESTNET)])
      expect(Object.keys(store.getState()[ChainId.TESTNET] ?? {})).toEqual([])
    })
  })
})
